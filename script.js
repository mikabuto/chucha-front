...
buttons.map( button => {
	button.addEventListener('click', (e) => {
		...
		if ((result.innerText == "Result" || result.innerText == "Wrong input!") && button_content != 'C') {
			tmp = "";
		}
		...
		if (typeof tmp === 'number') {
			let num_of_dig = (tmp.toString().includes('.')) ? (tmp.toString().split('.').pop().length) : (0);
			if (num_of_dig > 13) {
				tmp = tmp.toString();
				let e_ind = tmp.search('e');
				let dot_ind = tmp.search('.');
				if (e_ind != -1 && dot_ind != -1) tmp = tmp.slice(0, dot_ind + 5) + tmp.slice(e_ind);
